...
def run_func(op_code_node):
    ...
    def cdr(node):
        l_node = run_expr(node.value.next)
        result = strip_quote(l_node).value
        return create_new_quote_list(result.next, True)

    def cons(node):
        total_node = run_expr(node.value.next)
        if total_node.type is not TokenType.LIST :
            head_node = Node(total_node.type,total_node.value)
            tail_node = strip_quote(total_node.next).value
            head_node.next = tail_node
        else :
            head_node = strip_quote(total_node.value)
            tail_node = strip_quote(total_node.next).value
            head_node.set_last_next(tail_node)
        return create_new_quote_list(head_node, True)

    def null_q(node):
        q_node = node.value.next
        q_node = strip_quote(q_node)
        if q_node.value is None :
    ...
    def atom_q(node):
        q_node = node.value.next
        q_node = strip_quote(q_node)
        if q_node.type is TokenType.LIST :
            if q_node.value is None :
                return Node(TokenType.TRUE)
            else :
                return Node(TokenType.FALSE)
