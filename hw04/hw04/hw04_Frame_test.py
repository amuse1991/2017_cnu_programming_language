...
def check_keyword(token): #토큰을 받아서 키워드면 TRUE 키워드가 아니면 FALSE
    """
    :type token:str
    :param token:
    :return:
...
def _get_keyword_type(token): #문자로 된 키워드를 받아 해당하는 키워드 번호를 반환
    return {
